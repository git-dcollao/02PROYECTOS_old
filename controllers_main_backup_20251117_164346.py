    ...
    try:
        ...
        if not proyectos:
            proyectos_query_equipo = db.session.query(Requerimiento).join(
                ...
                Requerimiento.id_estado == 3  # Solo proyectos en ejecución
            ).distinct()
...
def proyectos_por_trabajador_all(trabajador_id):
    """Obtener TODOS los proyectos disponibles en el sistema para un trabajador (versión completa)"""
    try:
        # Verificar que el trabajador existe
        trabajador = Trabajador.query.get_or_404(trabajador_id)
        
        # Obtener TODOS los proyectos activos del sistema
        proyectos_query = db.session.query(Requerimiento).filter(
            Requerimiento.id_estado.in_([2, 3])  # Estados: En Desarrollo - Preparación y En Desarrollo - Ejecución
        ).order_by(Requerimiento.nombre)
        ...
        for proyecto in proyectos:
            ...
            # Verificar si el trabajador está asignado al proyecto
            esta_asignado = db.session.query(EquipoTrabajo).filter_by(
                id_trabajador=trabajador_id,
                id_requerimiento=proyecto.id
            ).first() is not None
...
def proyectos_estado_4():
    """Obtener todos los proyectos (requerimientos) con estado = 4"""
    try:
...
@controllers_bp.route('/procesar-proyecto-xlsx', methods=['POST'], endpoint='procesar_proyecto_xlsx')
def procesar_proyecto_xlsx():
    ...
    try:
        ...
        if hasattr(procesar_proyecto_xlsx, 'actividades_temp'):
            delattr(procesar_proyecto_xlsx, 'actividades_temp')
            print("🧹 Variables temporales limpiadas al inicio")
        ...
        for index, row in df_ordenado.iterrows():
            try:
                ...
                if not hasattr(procesar_proyecto_xlsx, 'actividades_temp'):
                    procesar_proyecto_xlsx.actividades_temp = []
                procesar_proyecto_xlsx.actividades_temp.append(nueva_actividad)
                ...
                print(f"❌ Error al analizar proyecto: {str(e)}")
                continue
        ...
        if proyectos_nuevos:
            ...
            # Obtener requerimientos disponibles para asignación
            ...
            return jsonify({
                'success': True,
                'message': f'Se detectaron {len(proyectos_nuevos_limpios)} proyectos nuevos para asignación',
                'proyectos_nuevos': proyectos_nuevos_limpios,  # USAR LISTA LIMPIA
                'requerimientos_disponibles': [
                    {'id': req.id, 'nombre': req.nombre} 
...
def limpiar_trabajadores_huerfanos():
    """
    Elimina trabajadores que no están asignados a ninguna actividad.
    Mantiene solo los trabajadores que realmente están siendo utilizados.
    """
    try:
        # Obtener todos los trabajadores que NO tienen avances de actividad
        trabajadores_sin_avances = db.session.query(Trabajador).filter(
            ~Trabajador.id.in_(
                db.session.query(AvanceActividad.trabajador_id).distinct()
            )
        ).all()
        
        if trabajadores_sin_avances:
            count_eliminados = len(trabajadores_sin_avances)
            
            # Eliminar trabajadores sin asignaciones
            for trabajador in trabajadores_sin_avances:
                db.session.delete(trabajador)
            
            db.session.commit()
            print(f"🧹 Eliminados {count_eliminados} trabajadores sin asignaciones de actividades")
            return count_eliminados
        else:
            print("✅ No hay trabajadores huérfanos para limpiar")
            return 0
            
    except Exception as e:
        db.session.rollback()
        print(f"⚠️ Error al limpiar trabajadores huérfanos: {e}")
        return 0

def extraer_y_crear_trabajadores_desde_recursos(recursos_string):
    """
    Extrae nombres de trabajadores de la cadena de recursos y los crea en la tabla trabajador si no existen.
    Solo guarda trabajadores que realmente serán asignados a tareas.
    
    Args:
        recursos_string (str): Cadena con recursos separados por comas/punto y coma
        
    Returns:
        list: Lista de objetos Trabajador creados o encontrados
    """
    if not recursos_string or recursos_string.strip() == '':
        return []
    
    trabajadores_procesados = []
    
    # Dividir recursos por comas o punto y coma
    recursos_lista = re.split(r'[,;]+', recursos_string)
    
    for recurso_item in recursos_lista:
        recurso_item = recurso_item.strip()
        if recurso_item:
            # Limpiar el nombre del recurso removiendo porcentajes y corchetes
            nombre_limpio = re.sub(r'\[.*?\]', '', recurso_item).strip()
            
            if nombre_limpio:  # Solo procesar si hay un nombre válido
                ...
                if trabajador_existente:
                    trabajadores_procesados.append(trabajador_existente)
                    print(f"✅ Trabajador existente encontrado: {trabajador_existente.nombre}")
                else:
                    # Crear nuevo trabajador solo si será asignado a una tarea
                    nuevo_trabajador = Trabajador(
                        nombre=nombre_limpio,
                        nombrecorto=nombre_limpio[:10],  # Abreviar para nombre corto
                        activo=True
                    )
                    ...
                    try:
                        db.session.add(nuevo_trabajador)
                        db.session.flush()  # Para obtener el ID sin hacer commit completo
                        trabajadores_procesados.append(nuevo_trabajador)
                        print(f"➕ Nuevo trabajador creado: {nuevo_trabajador.nombre} (ID: {nuevo_trabajador.id})")
                    except Exception as e:
                        print(f"⚠️ Error al crear trabajador {nombre_limpio}: {e}")
                        db.session.rollback()
    
    return trabajadores_procesados

def crear_avances_actividad(requerimiento_id, actividad_id, recursos_string, progreso_actual=0.0):
    """
    Crea o actualiza registros de avance_actividad para cada trabajador asignado a una actividad.
    IMPORTANTE: Limpia asignaciones anteriores y crea solo las nuevas basadas en recursos_string.
    """
    if not recursos_string or recursos_string.strip() == '':
        return []
    
    # PASO 1: Limpiar asignaciones anteriores para esta actividad
    asignaciones_anteriores = AvanceActividad.query.filter_by(
        actividad_id=actividad_id
    ).all()
    
    if asignaciones_anteriores:
        print(f"🧹 Limpiando {len(asignaciones_anteriores)} asignaciones anteriores de actividad ID {actividad_id}")
        for asignacion in asignaciones_anteriores:
            db.session.delete(asignacion)
        db.session.flush()  # Aplicar eliminaciones antes de crear nuevas
    
    avances_creados = []
    fecha_actual = datetime.now().date()
    
    print(f"📋 Creando nuevas asignaciones para actividad ID {actividad_id} con recursos: '{recursos_string}'")
    
    # PASO 2: Dividir recursos por comas o punto y coma
    recursos_lista = re.split(r'[,;]+', recursos_string)
    
    for recurso_item in recursos_lista:
        recurso_item = recurso_item.strip()
        if recurso_item:
            # Extraer porcentaje de asignación si existe
            porcentaje_match = re.search(r'\[(\d+(?:\.\d+)?)%\]', recurso_item)
            porcentaje_asignacion = float(porcentaje_match.group(1)) if porcentaje_match else 100.0
            
            # Limpiar el nombre del recurso removiendo porcentajes y corchetes
            nombre_limpio = re.sub(r'\[.*?\]', '', recurso_item).strip()
            
            # Buscar el trabajador correspondiente con coincidencia exacta
            trabajador = Trabajador.query.filter(
                (Trabajador.nombre == nombre_limpio) |
                (Trabajador.nombrecorto == nombre_limpio)
            ).first()
            
            if not trabajador:
                # Si no hay coincidencia exacta, intentar búsqueda flexible
                trabajador = Trabajador.query.filter(
                    (Trabajador.nombre.ilike(f'%{nombre_limpio}%')) |
                    (Trabajador.nombrecorto.ilike(f'%{nombre_limpio}%'))
                ).first()
            ...
            if trabajador:
                # Crear nuevo registro de avance (ya limpiamos los anteriores)
                nuevo_avance = AvanceActividad(
                    requerimiento_id=requerimiento_id,
                    actividad_id=actividad_id,
                    trabajador_id=trabajador.id,
                    porcentaje_asignacion=porcentaje_asignacion,
                    progreso_actual=progreso_actual,
                    progreso_anterior=0.0,  # Primera vez, progreso anterior es 0
                    fecha_registro=fecha_actual,
                    fecha_creacion=datetime.utcnow(),
                    fecha_actualizacion=datetime.utcnow()
                )
                db.session.add(nuevo_avance)
                avances_creados.append(nuevo_avance)
                print(f"✅ Asignado trabajador ID {trabajador.id} '{trabajador.nombre}' con {porcentaje_asignacion}%")
            else:
                print(f"⚠️ No se encontró trabajador para el recurso: '{nombre_limpio}'")
    
    print(f"🎯 Total asignaciones creadas: {len(avances_creados)}")
    return avances_creados

def procesar_recursos_actividad(recursos_string):
    """
    Procesa la cadena de recursos y crea/asigna trabajadores
    Retorna una lista de recursos procesados con sus IDs de trabajador
    """
    recursos_procesados = []
    
    if not recursos_string or recursos_string.strip() == '':
        return recursos_procesados
        return recursos_procesados
    
    # Dividir recursos por comas o punto y coma
    recursos_lista = re.split(r'[,;]+', recursos_string)
    
    for recurso_nombre in recursos_lista:
        recurso_nombre = recurso_nombre.strip()
        if recurso_nombre:
            # Limpiar el nombre del recurso removiendo porcentajes y corchetes
            nombre_limpio = re.sub(r'\[.*?\]', '', recurso_nombre).strip()
            
            # Buscar trabajador existente por nombre limpio (coincidencia parcial)
            trabajador = Trabajador.query.filter(
                Trabajador.nombre.ilike(f'%{nombre_limpio}%')
            ).first()
            ...
            if not trabajador:
                # Crear nuevo trabajador si no existe
                nombre_corto = nombre_limpio[:20] if len(nombre_limpio) > 20 else nombre_limpio
                email_safe = re.sub(r'[^a-zA-Z0-9]', '.', nombre_limpio.lower())
                
                trabajador = Trabajador(
                    nombre=nombre_limpio,
                    nombrecorto=nombre_corto,
                    profesion='Recurso del Proyecto',
                    email=f'{email_safe}@proyecto.local',
                    activo=True
                )
                db.session.add(trabajador)
                db.session.flush()  # Para obtener el ID
                
                print(f"Nuevo trabajador creado: {nombre_limpio} (ID: {trabajador.id})")
            else:
                print(f"Trabajador existente encontrado: {nombre_limpio} -> {trabajador.nombre} (ID: {trabajador.id})")
            
            recursos_procesados.append({
                'nombre_recurso': nombre_limpio,
                'trabajador_id': trabajador.id,
