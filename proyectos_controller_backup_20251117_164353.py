            ...
            try:
                ...
                if not trabajador:
                    trabajador = Trabajador(
                        nombre=recurso,
                        email=f"{recurso.lower().replace(' ', '.')}@temp.com",
                        rut=f"temp-{hash(recurso) % 100000000}",
                ...
                avance = AvanceActividad(
                    ...
                    observaciones='Creado automáticamente desde Excel'
                )
                db.session.add(avance)
                
            except Exception as e:
                logger.error(f"Error al crear avance para recurso '{recurso}': {str(e)}")
                continue
        ...
        if proyectos_nuevos:
            ...
            return jsonify({
                ...
                'requerimientos_disponibles': [
                    {
                        'id': req.id, 
                        'titulo': f"REQ-{req.id}: {req.titulo}"  # CORREGIDO: cambiar nombre a titulo
                    }
        ...
        try:
            for edt_nombre_tarea, requerimiento_id in asignaciones.items():
                ...
                if requerimiento:
                    ...
                    # Solo procesar actividades si este requerimiento fue seleccionado para procesar
                    procesar_actividades = (requerimiento_para_procesar is not None and 
                                           int(requerimiento_id) == int(requerimiento_para_procesar))
                    
                    print(f"   🔍 DEBUG: requerimiento_id={requerimiento_id}, para_procesar={requerimiento_para_procesar}, ¿procesar?={procesar_actividades}")
                    ...
                    if procesar_actividades:
                        ...
                        # Solo actualizar la asignación sin procesar actividades
                        print(f"   ⏭️ Proyecto '{nombre_proyecto_real}' asignado a requerimiento {requerimiento_id} (sin procesar actividades - no seleccionado)")
